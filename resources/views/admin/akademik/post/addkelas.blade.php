            ...
            <div class="card-body md:py-3 py-5">
                <div class="flex items-center grid grid-cols-12 gap-6">
                    <div class="col-span-9">
                        <h4 class="font-semibold text-xl text-dark dark:text-white mb-3">Form Tambah Data Kelas</h4>
                                ...
                                <form action="{{ route('kelas.store') }}" method="POST">
                                    @csrf
                                        ...
                                        <div class="relative">
                                            ...
                                            autofill:pt-6
                                            autofill:pb-2">
                                            <option selected>Pilih Matkuliah </option>
                                            @foreach($matkul as $mt)
                                            <option value="{{ $mt->matakuliah_id }}">[{{ $mt->kode_matakuliah }}] {{ $mt->nama_matakuliah }} [{{ $mt->prodi->nama_prodi }}]</option>
                                            @endforeach
                                            </select>
                                        ...
                                        <div class="relative mt-3">
                                            ...
                                            @foreach($dosen as $ds)
                                            <option value="{{ $ds->dosen_id }}">[{{ $ds->nidn }}] {{ $ds->nama_lengkap }} [{{ $ds->prodi->nama_prodi }}]</option>
                                            @endforeach
                                            </select>
                                            <label for="form_InputSelect3" class="absolute top-0 start-0 p-4 h-full truncate pointer-events-none transition ease-in-out duration-100 border border-transparent dark:text-white peer-disabled:opacity-50 peer-disabled:pointer-events-none
                                            peer-focus:text-xs
                                      ...
                                      @enderror
                                        ...
                                        <div class="relative  mb-3 mt-3">
                                            ...
                                            <label for="hs-floating-input-tahun_ajaran" class="absolute top-0 start-0 p-2.5 h-full text-sm truncate pointer-events-none transition ease-in-out duration-100 border border-transparent dark:text-white peer-disabled:opacity-50 peer-disabled:pointer-events-none
                                            peer-focus:text-xs
                                            peer-focus:-translate-y-1.5
                                            peer-focus:text-gray-500
                                            peer-[:not(:placeholder-shown)]:text-xs
                                        ...
                                        <div class="flex gap-3 justify-start mt-6">
                                            ...
                                            <a class="btn btn-light-error" href="{{ url('kelas') }}">Batal</a>
                                           </div>
