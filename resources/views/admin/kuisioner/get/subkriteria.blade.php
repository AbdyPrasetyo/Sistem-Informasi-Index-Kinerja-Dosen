            ...
            <div class="card-body md:py-3 py-5">
                <div class="flex items-center grid grid-cols-12 gap-6">
                    <div class="col-span-9">
                        <h4 class="font-semibold text-xl text-dark dark:text-white mb-3">Data Sukriteria Kuisioner Universitas
                            Janabadra</h4>
                    ...
                    <div class="lg:col-span-4 md:col-span-12 sm:col-span-12 col-span-12 ">
                        <form class="relative max-w-64">
                            <input type="text" class="form-control product-search pl-11 py-2" id="input-search"
                                placeholder="Search Subkriteria..." />
                            <i
                    ...
                    <div class="lg:col-span-8 md:col-span-12 sm:col-span-12 col-span-12">
                        <div class="flex justify-end items-center gap-3">
                            ...
                            <a href="{{ route('subkriteria.create') }}"
                                ...
                                <i class="ti ti-users text-white text-lg leading-none"></i> Tambah
                                 Sub Kriteria
                            </a>
                        </div>
                    </div>
...
@endphp
                                    ...
                                    <tr class="search-items">
                                        ...
                                        @if ($sbk->kriteria_id != $currentKriteriaId)
                                        @php
                                            $currentKriteriaId = $sbk->kriteria_id;
                                        @endphp
                                        <td
                                            class="usr-ph-no text-sm whitespace-nowrap text-bodytext dark:text-darklink p-4">
                                            {{ $sbk->kriteria->nama_kriteria }}
                                        </td>
                                        @else
                                        <td
                                            class="usr-ph-no text-sm whitespace-nowrap text-bodytext dark:text-darklink p-4">
                                        ...
                                        <td
                                            class="usr-ph-no text-sm whitespace-nowrap text-bodytext dark:text-darklink p-4">
                                            {{ $sbk->nama_subkriteria }}
                                        </td>

                                        <td class="text-sm whitespace-nowrap text-bodytext dark:text-darklink p-4">
                                            <div class="action-btn flex gap-3">
                                                <a href="{{ route('subkriteria.edit', $sbk->subkriteria_id) }}" class="text-info edit">
                                                    <i class="ti ti-pencil text-lg"></i>
                                                </a>

                                                <form action="{{ route('subkriteria.destroy', $sbk->subkriteria_id) }}"
                                                    method="POST" id="delete-form-{{ $sbk->subkriteria_id }}">
                                                    @csrf
                                                    @method('DELETE')

                                                    <button type="button" class="text-dark delete-btn" data-id="{{ $sbk->subkriteria_id }}">
                                                        <i
                            ...
                            <div class="flex justify-center items-center gap-x-5 mt-4">
                                <div class="pagination-wrapper">
                                    {{ $subkriteria->links('vendor.pagnation.custom') }}
                                </div>
                                <!-- Go To Page -->
        ...
        <script>
            $(document).on('click', '.delete-btn', function () {
             var subkriteria_id = $(this).data('id');
             Swal.fire({
                 ...
                 if (result.isConfirmed) {
                     ...
                     $('#delete-form-' + subkriteria_id).submit();
                 }
             });
         });
         </script>
